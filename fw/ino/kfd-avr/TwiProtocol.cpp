...
#define BIT_TIME F_CPU/4000 * TIME_OFFSET
#define HALF_BIT_TIME BIT_TIME/2 * TIME_OFFSET
#define SIG_TIME BIT_TIME*4 * TIME_OFFSET

// macro to clear interrupts
// the actual INT0/INT1 mapping is different on each CPU, so we should remap accordingly:
...
const uint8_t byteReverseTable[] PROGMEM = {
...
uint8_t reverseByte(uint8_t b)
{
    ...
    return pgm_read_word_near(&byteReverseTable[b]);
    //return table[b];
}
...
void twiResume(void) {
    ENABLE_KFD_RX_INT
    // clear out any bytes we may have received
    hasReceived = 0;
    RXByte = 0;
}
