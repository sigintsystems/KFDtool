...
// function to be called at mode entry
void usbModeInit(void){
    spConnect();
    halActLedOn();
}

// function to be called at mode exit
void usbModeCleanup(void){
    spDisconnect();
    halActLedOff();
}

// function to be called every loop
void usbModeLoop(void) {
        ...
        if (cmdData[0] == CMD_READ) // read
        {
            if (cmdCount == 2)
            {
                ...
                if (cmdData[1] == READ_KEY_INFO)
                {
                    ...
                    result = idReadKey(cmdData[2], &flags, &ckr, &kid, &aid, key);
                    if (result == 1)
                    {
                        #if defined(ENABLE_KEY_READOUT)
                        uint8_t rspData[40];
                        #else
                        uint8_t rspData[8];
                        #endif
                        ...
                        rspData[1] = cmdData[2]; // slot
                        rspData[2] = flags;
                        rspData[3] = (ckr >> 8) & 0xFF;
                        rspData[4] = (ckr & 0xFF);
                        rspData[5] = (kid >> 8) & 0xFF;
                        rspData[6] = kid & 0xFF;
                        rspData[7] = aid;
            ...
            if (cmdCount > 1)
            {
                ...
                else if (cmdData[1] == WRITE_KEY)
                {
                    if (cmdCount == 41)
                    {
                        ...
                        uint16_t ckr;
                        uint16_t kid;
                        // reassemble the kid and ckr to uint16's
                        ckr |= cmdData[4] << 8;
                        ckr |= cmdData[5];
                        kid |= cmdData[6] << 8;
                        kid |= cmdData[7];
                        ...
                        uint8_t key[32];
                        for (int i=0; i<32; i++) {
                            // assign all incoming bytes to the key array
                            key[i] = cmdData[9+i];
                        }

                        result = idWriteKey(cmdData[2], cmdData[3], ckr, kid, cmdData[8], key);
